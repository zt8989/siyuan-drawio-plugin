...
## 更新日志
- **v1.0.40**
  - 修复 draw.io 编辑器语言未跟随思源语言的问题（BCP 47：zh-CN 等现在正确映射到 draw.io 语言，不再回退到英文）
- **v1.0.39**
  - 增加默认保存路径设置（Petal 目录 vs Assets 目录）
  - 开发环境下每次重建自动复制 client 文件到 webapp 目录，实现即时预览
  - 新增 Electron 应用 CDP 调试技能
  - 修复设置弹窗挂载及 UI 问题
  - 移除已弃用的自定义库设置，精简为保存设置
- **v0.1.36 (2025-11-19)**
  - 增加移动端支持
- **v0.1.35 (2025-11-10)**
  - 增加图片右键编辑drawio支持
